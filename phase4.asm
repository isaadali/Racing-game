...
; Data section
car_color: db 0
msg1: db 'F1 coal edition', 0
msg2: db 'Press any key to continue', 0
msg3: db '24L-0652 Saad Ali', 0
msg4: db '24L-0745 Saad Sohail', 0
msg5: db 'Developed by:',0
msg6: db 'Best of Luck!',0
...
temp_row db 0
temp_col db 0
name_buffer: times 50 db 0
roll_buffer: times 50 db 0
...
.input_loop:
    ...
    ; Check for valid printable characters (space to ~)
    cmp al, 32
    jb .input_loop
    cmp al, 126
    ja .input_loop
    
    cmp cx, 49          ; Max length
    jge .input_loop
    
    ; Store character using stosb
    stosb
    inc cx
...
; Draw horizontal line for circle filling
; BX = start X, DX = Y, CX = end X, AL = color
draw_h_line:
    push bx
    push cx
.line_loop:
...
; Subroutine: draw_filled_circle
; Input: BX = center X, DX = center Y, CX = radius, AL = color
draw_filled_circle:
    push ax
    push bx
    push cx
    ...
    mov [temp_color], al    ; Save color
    mov si, 0               ; x = 0
    mov di, cx              ; y = radius
    
    ; Calculate decision = 3 - 2*radius
    mov ax, 3
    mov bp, cx
    shl bp, 1
    sub ax, bp
    mov [circle_decision], ax
    
.circle_loop:
    ; Draw 4 horizontal lines for filled circle
    push bx
    push dx
    
    ; Line 1: from (cx-x, cy+y) to (cx+x, cy+y)
    mov ax, bx
    sub ax, si              ; start_x = cx - x
    mov bp, bx
    add bp, si              ; end_x = cx + x
    mov bx, ax
    mov cx, bp
    add dx, di              ; y = cy + y
    mov al, [temp_color]
    ...
    ; Line 2: from (cx-x, cy-y) to (cx+x, cy-y)
    mov ax, bx
    sub ax, si
    mov bp, bx
    add bp, si
    ...
    ; Line 3: from (cx-y, cy+x) to (cx+y, cy+x)
    mov ax, bx
    sub ax, di              ; start_x = cx - y
    mov bp, bx
    add bp, di              ; end_x = cx + y
    mov bx, ax
    mov cx, bp
    add dx, si              ; y = cy + x
    mov al, [temp_color]
    ...
    ; Line 4: from (cx-y, cy-x) to (cx+y, cy-x)
    mov ax, bx
    sub ax, di
    mov bp, bx
    add bp, di
...
; BX = X position, DX = Y position, SI = string offset, AL = color
print_string_gfx:
    push ax
    push bx
    push cx
...
.next_char:
    ...
    ; Draw character using BIOS function 09h (write character and attribute)
    push ax
    push bx
    push cx
    push dx
    ...
    mov bl, [temp_color]   ; Color/attribute (with blink bit if set)
...
; Set cursor position in graphics mode
; DH = row, DL = column
set_cursor:
    push ax
    push bx
...
; BX = X position (column), DX = Y position (row), AL = car body color
draw_car:
    push ax
    push bx
    push cx
    ...
    mov si, bx          ; SI = starting X
    mov [car_color], al ; Save car body color
    push dx             ; Save starting Y
    ...
    mov cx, 6           ; Width
    mov di, 4           ; Height
    mov al, 1           ; Blue windshield
    call draw_rect
    
    ; Draw racing stripes (center line) - FIXED VERSION
    mov bx, si
    add bx, 7           ; Center X of car
    pop dx              ; Get original Y position
    push dx             ; Save it again for final cleanup
    add dx, 9           ; Start below windshield
    
    ; First stripe segment
    mov cx, 2           ; Width
    mov di, 2           ; Height
    mov al, 15          ; White
    call draw_rect
    ...
    ; Third stripe segment
    mov cx, 2
    mov di, 2
    mov al, 15
    call draw_rect
    
    ; Final cleanup - pop the saved Y position
    pop dx
...
start:
 ...
 ; Print title at top
    ...
    ; Print "Press any key" message with BLINKING YELLOW
    mov dh, 17; Row
    mov dl, 8          ; Column
    call set_cursor
    mov si, msg2
    mov al, 14          ; Yellow color (14)
    call print_string_gfx
    ...
    ; Draw three filled red circles (Formula 1 style)
    ; Circle 1 (left)
    mov bx, 130        ; Center X
    mov dx, 55         ; Center Y
    mov cx, 10          ; Radius
    ...
    ; Draw the cars
    mov bx, 165        ; Column (X position)
    mov dx, 90; Row (Y position)
    mov al, 14          ; Yellow color
    call draw_car
    
    mov bx, 135          ; Column (X position)
    mov dx, 90 ; Row (Y position)
 ...
 call clear_screen
  ; Ask for name in graphics mode
    ...
    ; Read name input (cursor is already positioned after prompt)
    mov di, name_buffer ; Set buffer for name
    call read_string_gfx
    ;roll no
    mov dh, 12          ; Row
    mov dl, 10          ; Column
  call set_cursor    
mov si, prompt_roll
    ...
    ; Read roll number input (cursor is already positioned after prompt)
    mov di, roll_buffer ; Set buffer for roll number
    call read_string_gfx
